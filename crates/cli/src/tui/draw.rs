...
use ratatui::{
    ...
    text::{Line, Span},
    widgets::{
        Block, Borders, BorderType, Clear, Gauge, List, ListItem, ListState, Paragraph, 
        Sparkline, Table, TableState, Tabs, Wrap,
    },
};

use super::{PeerRow, View};
...
pub fn draw_header_tabs(
    ...
    // Tab navigation
    let tab_titles = vec![
        "📊 Overview", "👥 Peers", "🚀 Apps", "🌐 Network", 
        "📝 Events", "📋 Logs", "⚙️  Actions"
    ];
...
pub fn draw_footer(f: &mut ratatui::Frame<'_>, area: Rect) {
    ...
    let help_text: Vec<Span> = shortcuts
        ...
        .flat_map(|(i, (key, desc))| {
            let mut spans = vec![
                Span::styled(key.to_string(), Style::default().fg(THEME_PRIMARY).add_modifier(Modifier::BOLD)),
                Span::styled(format!(" {}", desc), Style::default().fg(THEME_MUTED)),
            ];
            if i < shortcuts.len() - 1 {
                spans.push(Span::styled("  │  ".to_string(), Style::default().fg(THEME_MUTED)));
            }
            spans
        })
        .collect();
...
pub fn draw_overview(
    ...
    // Cluster Health Card
    let health_color = if peer_count > 0 { THEME_SUCCESS } else { THEME_ERROR };
    ...
    let comp_color = if drift == 0 { THEME_SUCCESS } else { THEME_WARNING };
    ...
    // System Stats Card
    let stats_color = if publish_errors_total > 10 { THEME_ERROR } else { THEME_PRIMARY };
    ...
    // CPU Sparkline
    let cpu_color = if cpu.last().unwrap_or(&0) > &80 { THEME_ERROR } else { THEME_SUCCESS };
    ...
    // Memory Sparkline
    let mem_color = if mem.last().unwrap_or(&0) > &80 { THEME_ERROR } else { THEME_PRIMARY };
    ...
    let events_items: Vec<ListItem> = events
        ...
        .map(|(i, (time, msg))| {
            ...
            let styled_line = Line::from(vec![
                ...
                Span::styled(msg, Style::default().fg(if i < 5 { THEME_TEXT } else { THEME_MUTED })),
            ]);
...
pub fn draw_peers(
    ...
    let healthy_peers = peers.values().filter(|p| p.last_ping.elapsed().as_secs() < 30).count();
    let total_drift: u64 = peers.values().map(|p| p.desired_components.saturating_sub(p.running_components)).sum();
    ...
    // Healthy Peers Card
    let health_color = if healthy_peers == total_peers && total_peers > 0 { THEME_SUCCESS } else { THEME_WARNING };
    let health_block = Block::default()
        .borders(Borders::ALL)
        .border_type(BorderType::Rounded)
        .border_style(Style::default().fg(health_color))
    ...
    // Drift Card
    let drift_color = if total_drift == 0 { THEME_SUCCESS } else { THEME_ERROR };
    ...
    // Avg RTT Card
    let rtt_color = if avg_rtt < 100 { THEME_SUCCESS } else if avg_rtt < 500 { THEME_WARNING } else { THEME_ERROR };
    ...
    // Peers table
    let cols = ["🏷️ Peer ID", "🔧 Version", "📡 RTT", "💓 Last Ping", "🏷️ Tags", "⚠️ Drift"];
    ...
    for (id, p) in peers.iter() {
        ...
        // Status indicators
        let ping_indicator = if secs < 10 { "🟢" } else if secs < 30 { "🟡" } else { "🔴" };
...
pub fn draw_logs(
    ...
    let status_indicator = if paused { "⏸️ PAUSED" } else { "▶️ LIVE" };
...
pub fn draw_component_logs(
    ...
    let log_items: Vec<ListItem> = lines
        ...
        .map(|(i, line)| {
            ...
            ListItem::new(Line::from(Span::styled(line, Style::default().fg(content_color))))
        })
        .collect();
    
    let selected_component = components.get(state.selected().unwrap_or(0))
        .map(|s| s.as_str())
        .unwrap_or("None");
...
pub fn draw_topology(
    ...
    let active_peers = topo.values().filter(|(_, last)| last.elapsed().as_secs() < 60).count();
    let has_addresses = topo.values().filter(|(addr, _)| addr.is_some()).count();
    let recent_discoveries = topo.values().filter(|(_, last)| last.elapsed().as_secs() < 10).count();
    ...
    // Active Peers
    let active_color = if active_peers > 0 { THEME_SUCCESS } else { THEME_WARNING };
    ...
    // Recent
    let recent_color = if recent_discoveries > 0 { THEME_SUCCESS } else { THEME_MUTED };
...
pub fn draw_wizard_dialog(f: &mut ratatui::Frame<'_>, area: Rect, title: &str, prompt: &str, input: &str, step: usize, total_steps: usize) {
    let popup_width = 60;
    let popup_height = 10;
