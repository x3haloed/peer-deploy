...
#[derive(Serialize)]
struct DeployEvent { component: String, digest: Option<String>, timestamp: u64 }
        ...
        for line in entries.iter().rev().take(200) {
            if let Some((ts_str, msg)) = line.split_once('|') {
                ...
                if msg.contains("deployed") {
                    let component = parse_after(msg, "component '").unwrap_or_else(|| "component".to_string());
                    let ts = ts_str.trim().parse::<u64>().unwrap_or(0);
                    out.push(DeployEvent { component, digest: None, timestamp: ts });
                }
            }
        }
    }
