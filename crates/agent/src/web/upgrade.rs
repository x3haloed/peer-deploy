...
pub async fn api_upgrade_multipart(mut multipart: Multipart) -> impl IntoResponse {
    ...
    for (idx, bin) in bins.into_iter().enumerate() {
        ...
        if let Err(e) = crate::cmd::upgrade(upload_path.display().to_string(), version, plat, vec![], vec![]).await {
            any_err = Some(format!("upgrade failed: {}", e));
            break;
        }
    }
    match any_err { None => (StatusCode::OK, "ok").into_response(), Some(e) => (StatusCode::BAD_REQUEST, e).into_response() }
}
