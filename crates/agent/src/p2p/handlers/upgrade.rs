...
pub async fn handle_upgrade(
    ...
    let sig = match base64::engine::general_purpose::STANDARD.decode(&pkg.signature_b64) {
        ...
        Err(e) => {
            push_log(
                &logs,
                "upgrade",
                format!("upgrade rejected (bad signature_b64: {e})"),
            )
            .await;
    ...
    let bin_bytes = match base64::engine::general_purpose::STANDARD.decode(&pkg.binary_b64) {
        ...
        Err(e) => {
            push_log(
                &logs,
                "upgrade",
                format!("upgrade rejected (bad binary_b64: {e})"),
            )
            .await;
    ...
    push_log(
        &logs,
        "upgrade",
        format!("verified signature and digest sha256={}", &digest[..16]),
    )
    .await;
    ...
    match binary_target_matches_host(&bin_bytes) {
        ...
        Err(err) => {
            push_log(
                &logs,
                "upgrade",
                format!("upgrade rejected (target mismatch: {})", err),
            )
            .await;
    ...
    if let Some(ref plat) = pkg.target_platform {
        ...
        if &host != plat {
            push_log(
                &logs,
                "upgrade",
                format!("upgrade rejected (platform {} != {})", host, plat),
            )
            .await;
            let _ = tx.send(Err(format!(
                "upgrade rejected (platform {} != {})",
                host, plat
            )));
            return;
        }
        ...
        Ok(mut f) => {
            ...
            if let Err(e) = f.write_all(&bin_bytes).await {
                push_log(
                    &logs,
                    "upgrade",
                    format!("upgrade rejected (write error: {e})"),
                )
                .await;
            ...
            if let Err(e) = f.sync_all().await {
                push_log(
                    &logs,
                    "upgrade",
                    format!("upgrade rejected (fsync file: {e})"),
                )
                .await;
            ...
            push_log(
                &logs,
                "upgrade",
                format!("wrote versioned binary to {}", versioned_path.display()),
            )
            .await;
        }
        Err(e) => {
            push_log(
                &logs,
                "upgrade",
                format!("upgrade rejected (open error: {e})"),
            )
            .await;
    ...
    push_log(
        &logs,
        "upgrade",
        format!("upgrade accepted v{} (prev v{})", pkg.version, previous),
    )
    .await;
    ...
    push_log(
        &logs,
        "upgrade",
        format!("spawning new process: {}", versioned_path.display()),
    )
    .await;
    ...
    if spawn_res.is_err() {
        push_log(
            &logs,
            "upgrade",
            "spawn failed; retaining old process and previous version",
        )
        .await;
