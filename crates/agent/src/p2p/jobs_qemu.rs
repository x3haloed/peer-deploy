...
use crate::policy::{load_policy, policy_enable_help, qemu_install_help, ExecutionPolicy};
...
pub async fn execute_qemu_job(
    ...
    let _ = job_mgr
        .add_job_log(
            job_id,
            "info".to_string(),
            format!("Staging QEMU target from {}", binary),
        )
        .await;
    ...
    let _ = job_mgr
        .add_job_log(
            job_id,
            "info".to_string(),
            "Executing under QEMU emulation".to_string(),
        )
        .await;
