...
pub async fn execute_oneshot_job(
    ...
    match job_runtime {
        ...
        common::JobRuntime::Native { binary, sha256_hex, args, env } => {
            ...
            let result = super::jobs_native::execute_native_job(&job_mgr, &job_id, &job, &binary, sha256_hex, args, env, &logs, None).await;
            match result {
                Ok(success_msg) => {
                    let _ = job_mgr.complete_job(&job_id, 0).await;
                    let _ = tx.send(Ok(success_msg));
        ...
        common::JobRuntime::Qemu { binary, sha256_hex, args, env, target_platform, qemu_binary } => {
            ...
            let result = super::jobs_qemu::execute_qemu_job(&job_mgr, &job_id, &job, &binary, sha256_hex, args, env, target_platform, qemu_binary, &logs, None).await;
            match result {
                Ok(success_msg) => {
                    let _ = job_mgr.complete_job(&job_id, 0).await;
                    let _ = tx.send(Ok(success_msg));
...
pub async fn execute_service_job(
    ...
    match job_runtime {
        ...
        common::JobRuntime::Native { binary, sha256_hex, args, env } => {
            push_log(&logs, &label, "dispatch: native".to_string()).await;
            let result = super::jobs_native::execute_native_job(&job_mgr, &job_id, &job, &binary, sha256_hex, args, env, &logs, Some(&mut cancel_rx)).await;
            match result {
                Ok(success_msg) => {
                    let _ = job_mgr.add_job_log(&job_id, "info".to_string(), "Service job completed normally".to_string()).await;
                    let _ = job_mgr.complete_job(&job_id, 0).await;
        ...
        common::JobRuntime::Qemu { binary, sha256_hex, args, env, target_platform, qemu_binary } => {
            push_log(&logs, &label, "dispatch: qemu".to_string()).await;
            let result = super::jobs_qemu::execute_qemu_job(&job_mgr, &job_id, &job, &binary, sha256_hex, args, env, target_platform, qemu_binary, &logs, Some(&mut cancel_rx)).await;
            match result {
                Ok(success_msg) => {
                    let _ = job_mgr.add_job_log(&job_id, "info".to_string(), "Service job completed normally".to_string()).await;
                    let _ = job_mgr.complete_job(&job_id, 0).await;
