...
use common::{MountKind, PackageComponent, PackageManifest, PackageMountSpec};
...
pub async fn package_create(
    ...
    let comp_name = name_override.unwrap_or_else(|| {
        dir_path
            .file_name()
    ...
    if dir_path.join("seed-data").exists() {
        mounts.push(PackageMountSpec {
            kind: MountKind::State,
            guest: "/data".to_string(),
            source: None,
            size_mb: None,
            volume: Some(comp_name.clone()),
            seed: Some("seed-data/".to_string()),
        });
    }
    ...
    let manifest = PackageManifest {
        component: PackageComponent {
            ...
            wasm: "component.wasm".to_string(),
            sha256: Some(wasm_sha256),
        },
        mounts,
    };
    ...
    let out_path = if let Some(out) = output {
        ...
        let filename = if base.ends_with(".realm") {
            base
        } else {
            format!("{}.realm", base)
        };
