...
use common::{
    serialize_message, sha256_hex, sign_bytes_ed25519, AgentUpgrade, Command, OwnerKeypair,
};
...
pub async fn upgrade_multi(
    ...
    if bins.is_empty() {
        let file =
            file.ok_or_else(|| anyhow::anyhow!("--file is required if no --bin provided"))?;
...
pub async fn upgrade(
    ...
    swarm.behaviour_mut().gossipsub.publish(
        topic_cmd.clone(),
        serialize_message(&Command::UpgradeAgent(pkg)),
    )?;
